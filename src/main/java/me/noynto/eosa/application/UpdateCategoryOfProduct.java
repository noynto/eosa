...
public record UpdateCategoryOfProduct(
    ...
    public Product handle(Command command) {
        ...
        Product product = this.productProvider.read(command.productId)
                .orElseThrow(() -> new RuntimeException("Le produit " + command.productId.value() + " sur lequel mettre à jour la catégorie n'existe pas."));
